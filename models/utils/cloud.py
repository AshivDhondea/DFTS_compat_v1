    ...
    for i in remote:
        ...
        for j in device:
            ...
            if curIn==out:
                ...
                r = tf.keras.layers.Input(out.shape[1:])
...
def remoteModel(model, split, custom_objects=None):
    ...
    RMCfg = createRMCfg(tf.keras.models.clone_model(model), remoteIns, deviceOuts, index)
    ...
    remoteModel = tf.keras.models.Model.from_config(RMCfg, custom_objects=custom_objects)
